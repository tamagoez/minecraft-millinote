...
export function HeadTitle({ children }) {
  return (
    <>
      <h1 className="font-bold text-2xl text-base-900 border-solid pb-2">{children}</h1>
    </>
  )
}
...
export function ContentH2({ children }) {
  return (
    <>
      <div className="border-dashed border-b-4 bg-base-500 pt-6 pb-2">
  ...
  return (
    <>
      <div className="rounded-lg bg-base-content pt-2 pb-4 shadow-md shadow-base-content/50">
        <code className="px-2 z-10 text-base-200 py-3 overflow-x-auto">{children}</code>
      </div>
    </>
  )
}
